...
Imager.prototype = {
  ...
  resizeFile: function (file, preset, filename, cb) {
    ...
    gm(file['path'])
      ...
      .quality(preset.size.quality || 100)
      .write(tempFile, function (err) {
        if (err) return cb(err);
        async.each(self.storage, function (storage, cb) {
          self['pushTo' + storage](tempFile, remoteFile, filename, ct, cb);
  ...
  cropFile: function (file, preset, filename, cb) {
    ...
    if (preset.size.crop.indexOf(' ') > -1) {
      gravity = preset.size.crop.split(' ')[1];
      preset.size.crop = preset.size.crop.split(' ')[0];
    }
  ...
  resizeAndCropFile: function (file, preset, filename, cb) {
    ...
    var gravity = preset.type.gravity || 'NorthWest';
    if (preset.type.crop.indexOf(' ') > -1) {
      preset.type.gravity = preset.type.crop.split(' ')[1];
    ...
    gm(file['path'])
      ...
      .quality(preset.type.quality || 100)
      .write(tempFile, function (err) {
        if (err) return cb(err);
        async.each(self.storage, function (storage, cb) {
          self['pushTo' + storage](tempFile, remoteFile, filename, ct, cb);
