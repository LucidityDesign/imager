        ...
        if (variant.resize) {
            Object.keys(variant.resize).forEach(function (name) {
                var processFiles = function (cb) {
                    var preset = {
                        name: name,
                        size: variant.resize[name].size,
                        sep: variant.separator || '_',
                        quality: variant.resize[name].quality || variant.quality || 100
                    };
        ...
        if (variant.crop) {
            Object.keys(variant.crop).forEach(function (name) {
                var processFiles = function (cb) {
                    var preset = {
                        name: name,
                        size: variant.crop[name].crop,
                        sep: variant.separator || '_',
                        quality: variant.crop[name].quality || variant.quality || 100
                    };
            ...
            Object.keys(variant.resizeAndCrop).forEach(function (name) {
                var processFiles = function (cb) {
                    var preset = {
                        ...
                        sep: variant.separator || '_',
                        quality: variant.resizeAndCrop[name].quality || variant.quality || 100
                    };
...
function getFileInfo(file, cb) {
    var f = {
        ...
        type: typeof (file) == 'string' ? mime.lookup(file) : mime.lookup(file.path),
